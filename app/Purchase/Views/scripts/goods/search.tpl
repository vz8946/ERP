			...
			<div id="selection_container" class="choose"
				...
				<ul class="choose_list" id="itemArray" style="clear:both;">
					...
					<li><a
						...
						target="_blank"> <img
							src="{{$imgBaseUrl}}/{{$data.goods_img|replace:'.':'_380_380.'}}"
							alt="{{$data.goods_name}}" /></a>
						<p style="height: 60px; padding-top: 8px;">
							<a
								href="/b-{{$data.as_name|default:'jiankang'}}/detail{{$data.goods_id}}.html">{{$data.goods_title}}
								{{$data.goods_style}} </a>
						</p> <span>{{$data.goods_alt}}</span> <em>市场价：￥{{$data.market_price}}</em>
						<br /> <span style="display: inline; color: #666;">内 购 价：</span><span
