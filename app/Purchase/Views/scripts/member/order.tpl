			...
			<table width="100%" cellspacing="0" cellpadding="0" border="0"  class="re_table">
				...
				<tbody>
					...
					<tr>
						...
						<td> {{if $order.status==0 and $order.status_logistic<5 and ($order.price_pay-($order.price_payed+$order.price_from_return)) > 0}}<a class="{{$order.batch_sn}}" href="/member/order-detail/batch_sn/{{$order.batch_sn}}">付款</a>{{/if}}
						{{if $order.price_payed eq 0 and $order.status eq 0 and $order.status_logistic eq 0 and $order.parent_batch_sn==''}}<a href="javascript:;" onclick="cancelOrder({{$order.batch_sn}});" class="{{$order.batch_sn}}" style="color:#999999;">取消订单</a>{{/if}}
