...
class GoodsController extends Zend_Controller_Action
{
	...
	{
		$this -> _api = new Purchase_Models_API_Goods();
    ...
    {
        $this -> _groupGoodsAPI = new Purchase_Models_API_GroupGoods();
        $this -> _msgAPI = new Purchase_Models_API_Msg();
       ...
       //库存
        ...
        $this -> view -> page_title = $data['meta_title'] ? $data['meta_title'] : $data['goods_name'].$data['goods_style'].'-'.$data['cat_name']."种子-【品质,价格,怎么样】 -垦丰商城";
        $this -> view -> page_keyword = $data['meta_keywords'] ? $data['meta_keywords'] : $data['goods_name'].','.$data['cat_name'].",".$data['goods_name'].'品质,'.$data['goods_name'].'价格';
        $this -> view -> page_description = $data['meta_description'] ? $data['meta_description'] : $data['goods_name'].'品质,价格-'.$data['cat_name'].'种子-垦丰电商-专业的种子商城提供'.$data['goods_name'].'在线销售!';
        ...
        $this->view->js_more=',goods.js,jquery.jqzoom.js,check.js';
        ...
        if ($number && ($product_sn || $id)) {
            ...
            if (!$stockAPI -> checkPreSaleProductStock($id, $number, true)) {
                ...
                if ($stock['able_number'] <= 0) {
                    $goodsDB = new Purchase_Models_DB_Goods();
    ...
    public function galleryAction()
    {
		...
		if(!empty($search['bid']) && empty($search['price'])){
			if(in_array($cat_id, $conf_scid)){
				...
				$meta_title = $seo_brand_name.'种子,0-2000元以上'.$cat_name.'种子价格,报价-垦丰商城';
			}
			...
			if(in_array($cat_id, $conf_scid)){
				...
				$meta_title = '种子,'.$seo_price_area.$cat_name.'种子价格,报价-垦丰商城';
			}
		}else{
		...
		if(!empty($search['bid']) && empty($search['price'])){
			...
			if(in_array($cat_id, $conf_scid)){
				...
				$meta_keywords = $seo_brand_name.'保健品,'.$seo_price_area.$cat_name.'保健品,'.$seo_brand_name.'保健品价格,'.$seo_brand_name.'保健品报价';
			}
			...
			if(in_array($cat_id, $conf_scid)){
				...
				$meta_keywords = '品牌保健品,'.$seo_price_area.$cat_name.'保健品,品牌保健品价格,品牌保健品报价';
			}
		}else{
		...
		if(!empty($search['bid']) && empty($search['price'])){
			if(in_array($cat_id, $conf_scid)){
				$meta_description = $seo_brand_name.$cat_name.',0-2000元以上'.$seo_brand_name.$cat_name.'产品，垦丰商城提供'.$seo_brand_name.$cat_name.','.$seo_brand_name.$cat_name.'产品在线销售,价格,报价,怎么样,好不好!';
			}else{
				$meta_description = $seo_brand_name.'种子,0-2000元以上'.$seo_brand_name.'种子，垦丰商城提供'.$seo_brand_name.'种子,'.$seo_brand_name.'种子在线销售,价格,报价!';
			}
		}elseif(!empty($search['bid']) && !empty($search['price'])){
			if(in_array($cat_id, $conf_scid)){
				...
				$meta_description = $seo_brand_name.'种子,'.$seo_price_area.$seo_brand_name.'种子，垦丰商城提供'.$seo_brand_name.'种子,'.$seo_brand_name.'种子在线销售,价格,报价，品质保证!';
			}
		}elseif(empty($search['bid']) && !empty($search['price'])){
			if(in_array($cat_id, $conf_scid)){
				$meta_description = '品牌'.$cat_name.','.$seo_price_area.'品牌'.$cat_name.'产品，垦丰商城提供'.'品牌'.$cat_name.',品牌'.$cat_name.'产品在线销售,价格,报价,怎么样,好不好，品质保证!';
			}else{
				$meta_description = '品牌种子,'.$seo_price_area.'品牌种子，垦丰商城提供品牌种子,品牌种子在线销售,价格,报价，品质保证!';
			}
		}else{
	...
	public function commentAction()
	{
        ...
        if($id){
			...
			$goods = new Purchase_Models_DB_Goods();
	...
	public function searchAction()
    {
        ...
        $this->view->list = $list;
        //SEO
        $this -> view -> page_title = $cat_name ? $cat_name ." 垦丰商城 " : (isset($search['keyword']) ? $search['keyword'] : '种子,玉米,水稻,大豆,小麦,大麦,甜菜,其他');
        $this -> view -> page_keyword = $cat['meta_keywords'] ? $cat['meta_keywords'] : $cat_name."种子,玉米,水稻,大豆,小麦,大麦,甜菜,其他";
        $this -> view -> page_description = $cat['meta_description'] ? $cat['meta_description'] : '提供'.$cat_name.'在线销售! ';
