...
class FlowController extends Zend_Controller_Action
{
	...
	public function init()
    {
	    ...
	    $_auth = Purchase_Models_API_Auth :: getInstance();
		$this -> _api = new Purchase_Models_API_Cart();
    ...
    public function delAddrAction() {    	
        ...
        $this -> _api -> setSetting('addr',null);
    ...
    public function editAddAddrAction() {
        ...
        $cartGoods = Purchase_Models_API_Cart :: makeCartGoodsToArray();
    ...
    public function indexAction()
    {
	   ...
	   if($cat_products['goods_amount']>10 && $cat_products['goods_amount']<199){
        ...
        $this -> view -> page_keyword = "在线购买,种子";
        $this -> view -> page_description = '垦丰电商 -专业的垦丰商城! ';
    }
    ...
    public function addrAction()
    {
        ...
        if ((isset($_COOKIE['cart']) && $_COOKIE['cart'] != '') || (isset($_COOKIE['p']) && $_COOKIE['p'] != '' || (isset($_COOKIE['groupgoods']) && $_COOKIE['groupgoods'] != ''))) { //购物车有商品
            
        	//验证购物车
        	$address = null;
            if ($this -> _request -> getParam('type', null) == 'checkout') {
                if ($this -> user['last_address_id'] && $this -> user['last_pay_type']) {
                	//默认选择 最后一次
                    $this -> _api -> setSetting('addr', $this -> user['last_address_id']);
                    $this -> _api -> setSetting('payment', $this -> user['last_pay_type']);
                    $address = array_shift($this -> _api -> getAddress(array('address_id' => $this -> user['last_address_id'],
                                                                             'member_id' => $this -> user['member_id'])));
                    if ($address['area_id']){
                        $this -> _api -> setSetting('logistic', $this -> _api -> getLogistic($address['area_id']));
                    ...
                    $cartGoodsArr = $this -> _api -> makeCartGoodsToArray();
                    $cart = $this -> _api -> getCartProduct($cartGoodsArr);
                    if ($cart['only_vitual']) {
                        header("Location: {$this -> getFrontController() -> getBaseUrl()}/flow/order/");
                        exit;
                    }
        	...
        	echo Zend_Json::encode(array('status'=>0,'url'=>'/flow','msg'=>'您的购物车内没有商品！'));
        	exit();
        }
    }
    ...
    public function setPaymentAction() {
        ...
        if($payment['img_url'])
        {
        	$tips .= '<div class="pay_method_selected">在线支付  <img  alt="'.$payment['name'].'" src="'.$payment['img_url'].'" /></div>';
        }else
        {
        	$tips .= '<div class="pay_method_selected">线下支付 '.$payment['name'].'</div>';
        } 
	...
	*/
	...
	public function setDeliveryAction()
	{
		...
		$tips = '<h2><b>配送与发票</b> <span class="step-action" id="invoice_edit_action"><a href="javascript:;" onclick="editDelivery()">修改</a></span></h2>  <div class="invoiceInfo"><p><b>配送方式</b>&nbsp;快递配送</p>';
		if($invoice_type == 1)
		{
		  $tips.='<p><b>发票信息</b>&nbsp;个人：'.$invoice_name.'</p><p><b>发票内容</b>&nbsp;保健品</p>';
		}elseif ($invoice_type == 2){
		  $tips.='<p><b>发票信息</b>&nbsp;单位：'.$invoice_name.'</p><p><b>发票内容</b>&nbsp;保健品</p>';
		}else{
    ...
    public function orderAction()
    {
	   ...
	   if($product['goods_amount']>0){
			if ($this -> _request -> isPost() && $paymentType = $this -> _request -> getParam('pay_type', null)) {
				$this -> _api -> setSetting('payment', $paymentType);
			}
			// 没有设置支付方式时就默认设置为用户的历史信息
			else if(!$this -> _api -> getSetting('payment') && $this -> user['last_pay_type']) { 
				$this -> _api -> setSetting('payment', $this -> user['last_pay_type']);
			}
			if (!$this -> _api -> getSetting('delivery') && $this -> user['delivery']) {
				$this -> _api -> setSetting('delivery', unserialize($this -> user['delivery']));
			}else{
				$delivery = array();
        ...
        if (!isset($shopConfig['min_point_to_price'])) {
            $shopConfig['min_point_to_price'] = 1000;
        }
        ...
        $this -> view -> page_keyword = "垦丰电商订单,种子";
    ...
    */
    ...
    public function addOrderAction()
    {
        ...
        if($fastTackBuy){
        	...
        	if ($product['only_vitual']) {
            	...
            	$address['phone'] =  $this -> _request -> getParam('phone',0);
            	...
            	if(Custom_Model_Check::isMobile($address['phone'])){
    ...
    public function giftCardAction()
    {
    	...
    	$this->view->address_id  =  $this -> _api -> getSetting('addr');
