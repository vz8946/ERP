...
class HelpController extends Zend_Controller_Action
{
	...
	{
        $this -> _pageAPI = new Purchase_Models_API_Page();
	...
	public function indexAction()
	{
	    ...
	    $this -> view -> page_description ="垦丰电商 -专业的种子产品商城，为消费者提供种子，种子,玉米,水稻,大豆,小麦,大麦,甜菜,其他，品质保证！";
	...
	public function listAction()
	{
        ...
        $this -> view -> page_keyword = $info['title'].',垦丰电商'.$info['title'].',种子商城'.$info['title']; 
        $this -> view -> page_description = '垦丰商城'.$info['title'].',垦丰电商种子商城'.$info['title'].',垦丰电商-专业的种子商城，品质保证!'; 
	...
	public function infoAction()
	{
        ...
        $this -> view -> page_keyword = "垦丰电商帮助中心,种子帮助中心,帮助中心";
    ...
    public function sitemapAction(){
        ...
        $apiGoods = new Purchase_Models_API_Goods();
    ...
    public function createSitemapAction(){
		...
		foreach ($arr_custom as $k => $v) {
	        ...
	        $item .= "\t".'</url>'."\n";
			$items[] = $item;
			$txt_items[] = 'http://jiankang/'.$v;
			$html_items[] = '<a href="http://jiankang/">垦丰</a>';
		}
        ...
        foreach ($this->view->tree_cat as $k=>$v){
            ...
            $item .= "\t".'</url>'."\n";
            $items[] = $item;
			$txt_items[] = 'http://www.1jiankang.com/gallery-'.$v['cat_id'].'.html';
			$html_items[] = '<a href="http://www.1jiankang.com/gallery-'.$v['cat_id'].'.html">'.$v['cat_name'].'</a>';
			
            if(!empty($v['sub'])){
                foreach ($v['sub'] as $kk=>$vv){
                	$item = '';
                    $item .= "\t".'<url>'."\n";
                    $item .= "\t"."\t".'<loc>http://www.1jiankang.com/gallery-'.$vv['cat_id'].'.html</loc>'."\n";
                    $item .= "\t"."\t".'<lastmod>'.$cdate.'</lastmod>'."\n";
                    $item .= "\t"."\t".'<changefreq>daily</changefreq>'."\n";
                    $item .= "\t".'</url>'."\n";
					$items[] = $item;
					$txt_items[] = 'http://www.1jiankang.com/gallery-'.$vv['cat_id'].'.html';
					$html_items[] = '<a href="http://www.1jiankang.com/gallery-'.$vv['cat_id'].'.html">'.$vv['cat_name'].'</a>';
        ...
        foreach ($list_brand as $k=>$v){
        	$item = '';
            $item .= "\t".'<url>'."\n";
            $item .= "\t"."\t".'<loc>http://www.1jiankang.com/b-'.$v['as_name'].'/</loc>'."\n";
            $item .= "\t"."\t".'<lastmod>'.$cdate.'</lastmod>'."\n";
            $item .= "\t"."\t".'<changefreq>daily</changefreq>'."\n";
            $item .= "\t".'</url>'."\n";
			$items[] = $item;
			$txt_items[] = 'http://www.1jiankang.com/b-'.$v['as_name'].'/';
			$html_items[] = '<a href="http://www.1jiankang.com/b-'.$v['as_name'].'/">'.$v['brand_name'].'</a>';
        }        
        ...
        foreach ($list_goods as $k=>$v){
            ...
            $item .= "\t".'<url>'."\n";
            $item .= "\t"."\t".'<loc>http://www.1jiankang.com/b-'.$v['as_name'].'/detail'.$v['goods_id'].'.html</loc>'."\n";
            $item .= "\t"."\t".'<lastmod>'.$cdate.'</lastmod>'."\n";
            $item .= "\t"."\t".'<changefreq>daily</changefreq>'."\n";
            $item .= "\t".'</url>'."\n";
			$items[] = $item;
			$txt_items[] = 'http://www.1jiankang.com/b-'.$v['as_name'].'/detail'.$v['goods_id'].'.html';
			$html_items[] = '<a href="http://www.1jiankang.com/b-'.$v['as_name'].'/detail'.$v['goods_id'].'.html">'.$v['goods_name'].'</a>';
        }        
        ...
        foreach ($list_group_goods as $k=>$v){
        	...
        	$v['as_name'] = empty($v['as_name']) ? '1jiankang' : $v['as_name'];
            $item .= "\t".'<url>'."\n";
            $item .= "\t"."\t".'<loc>http://www.1jiankang.com/groupgoods-'.$v['group_id'].'.html</loc>'."\n";
            $item .= "\t"."\t".'<lastmod>'.$cdate.'</lastmod>'."\n";
            $item .= "\t"."\t".'<changefreq>daily</changefreq>'."\n";
            $item .= "\t".'</url>'."\n";
			$items[] = $item;
			$txt_items[] = 'http://www.1jiankang.com/groupgoods-'.$v['group_id'].'.html';
			$html_items[] = '<a href="http://www.1jiankang.com/groupgoods-'.$v['group_id'].'.html">'.$v['group_goods_name'].'</a>';
        }        
        ...
        foreach ($list_help_center as $k=>$v){
			...
			foreach ($v['list'] as $kk => $vv) {
				$item = '';
	            $item .= "\t".'<url>'."\n";
	            $item .= "\t"."\t".'<loc>http://www.1jiankang.com/help/detail-'.$vv['article_id'].'.html</loc>'."\n";
	            $item .= "\t"."\t".'<lastmod>'.$cdate.'</lastmod>'."\n";
	            $item .= "\t"."\t".'<changefreq>daily</changefreq>'."\n";
	            $item .= "\t".'</url>'."\n";
				$items[] = $item;
				$txt_items[] = 'http://www.1jiankang.com/help/detail-'.$vv['article_id'].'.html';
				$html_items[] = '<a href="http://www.1jiankang.com/help/detail-'.$vv['article_id'].'.html">'.$vv['title'].'</a>';
			}			
        }        
