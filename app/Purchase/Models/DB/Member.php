...
class Purchase_Models_DB_Member
{
	/**
     * Zend_Db
     * 
    ...
    public function updateMemberCartInfo($data, $id)
    {
		$where = $this -> _db -> quoteInto('user_id = ?', $id);
		$memberSet = array (
                      'last_address_id' => $data['last_address_id'],
                      'last_pay_type' => $data['last_pay_type']
                      );
        return $this -> _db -> update($this -> _table, $memberSet, $where);
    }
	/**
     ...
     * @return   array
     */
	public function getGiftCardLog($user_id, $page, $pageSize)
	{
		$pageSize = ((int)$pageSize > 0) ? (int)$pageSize : $this -> _pageSize;
		if ($page!=null) {
		    $offset = ($page-1)*$pageSize;
        ...
        $whereSql="  where user_id= ".$user_id;
        $logsqlcount = "SELECT count(*) from ".$this->_tableGiftCardLog.$whereSql ;
        $logsql = "SELECT * from ".$this->_tableGiftCardLog.$whereSql ;
