...
class Shop_Models_API_GroupGoods
{
    /**
     * 
     * @var Shop_Models_DB_GroupGoods
     */
	public $_db = null;
	...
	{
		$this -> _db = new Shop_Models_DB_GroupGoods();
	...
	public function responseToCart($datas)
	{
		if($_COOKIE['groupgoods']){
			...
			foreach ($tmp as $k =>$v){
				...
				//站外专享活动
				$offerAPI = new Shop_Models_API_Offers();
				$offersList = $offerAPI -> getAllActiveOffers(array('group-exclusive'));
				if ($offersList) {
        			foreach ($offersList as $key => $offers) {
        				$offersPlugin = $offerAPI -> getClass($offers['offers_type']);
        			    $groupGoodsOne = $offersPlugin -> responseToCart($offers, $groupGoodsOne);
        			    if ( $groupGoodsOne['offers'][$offers['offers_id']] ) {
        			        $datas['offers'][$offers['offers_id']][] = $groupGoodsOne['offers'][$offers['offers_id']];
        			    }
        			}
        		}
        		...
        		if($action == 'index')
        		{
		            ...
		            <td class="s_del"><a href="javascript:void(0);" onclick="delGroupGoods('.$k.')">删除</a></td>
						...
						foreach ($groupGoodsOneConfig as $kk => $vv){
							$rowSpan = '';
							if($len>0 && $j==1) $rowSpan = '<td rowspan="'.$len.'"><b class="c2">组合商品</b></td>';
        		...
        		foreach ($groupGoodsOneConfig as $kk => $vv){
        			...
        			$html .= '<tr id="groupgoods_'.$i.'_'.$j.'">
        			'.$rowSpan.'	
				<td class="t_pro"><img src="/'.str_replace('.', '_60_60.', $vv['goods_img']).'" border="0" width="60" height="60">'.$vv['goods_name'].'</td>
				<td class="td_right" align="center">&nbsp;</td>
				<td align="center" id="buy_number_groupgoods_'.$i.'_'.$j.'"><input type="hidden" id="buy_number_groupgoods_'.$i.'_'.$j.'" value="'.$vv['number']*$v[0].'" />'.$vv['number'].'&times;'.$v[0].'</td>
				<td class="td_right" align="center" colspan="3">&nbsp;</td>
				</tr>';
