<?php
class Shop_Models_API_Goods extends Custom_Model_Dbadv
{
    /**
     *
	...
	public function get($where = null, $fields = '*', $page=null, $pageSize = null, $orderBy = null, $dooffer = true)
	{
        ...
        if ( $dooffer ) {   //优惠活动
    		$offers = new Shop_Models_API_Offers($this);
	...
	public function getGoodsTag($where,$offers=true)
	{
        ...
        if(count($goodsData)>0){
			...
			if($goods_ids){
				 $goodsdetails = $this ->get("a.goods_id in($goods_ids)", 'a.goods_id,a.view_cat_id,b.cat_path,a.goods_name,a.market_price,a.staff_price,a.goods_sn,a.price,a.goods_img,goods_style,a.goods_alt,d.as_name', $page, $pageSize,'find_in_set(a.goods_id,"'.$goods_ids.'")',$offers);
			}
			if($brand_ids){
	...
	public function view($id, $top_attr_id = 0)
	{
        ...
        if ($datas) {
            ...
            //优惠活动
            $offers = new Shop_Models_API_Offers($this);
            $datas = $offers -> responseToView($datas);
	...
	public function getOfferPriceById($gid){
        ...
        //优惠活动
        $offers = new Shop_Models_API_Offers($this);
        $datas = $offers -> responseToView($list_goods);
		
		return $datas[0]['price'];
