...
define('POINT_AUTH_EMAIL_TYPE', 11); //邮箱验证 积分类型
 class Shop_Models_API_Member extends Custom_Model_Dbadv
 {
 	/**
     * 会员帐户 DB
     * 
    ...
    public function register($data , $needFilter = true)
    {
        ...
        if (!$data['parent_id']) {
            ...
            $pinfo = $union -> getUidFromCookie(true);
            if ($pinfo) {
                $data['parent_id'] = $pinfo['user_id'];
                $data['parent_user_name'] = $pinfo['user_name'];
                $parentParam = $union -> getNidFromCookie();
                $parentParam && $data['parent_param'] = $parentParam;
            }
        }
        $data['discount'] = 1;
	...
	public function getWareHouseINfoByWareHouseId($warehouse_id){
		if(intval($warehouse_id)<1){
			return 0;
		}
		return $this->_orderDb->getWareHouseINfoByWareHouseId($warehouse_id);
	}
    ...
    public function union($batchSN)
    {
        ...
        $order = array_shift($this -> _orderDb -> getOrderBatch(array('batch_sn' => $orderSN)));//取第一条订单记录
        ...
        $unionObj = new Shop_Models_API_Union();
        $unionObj -> setAffiliate($data);
    }
    ...
    public function getOrderBatchGoods1($where)
    {
        ...
        if ($data) {
            ...
            foreach ($data as $k => $v) {
                ...
                if ($v['offers_type'] != 'fixed' and $v['offers_type'] != 'discount' and $v['offers_type'] != 'exclusive'  and $v['offers_type'] != 'price-exclusive') {//特价 和 折扣 和 专享 不参与统计
                    $forCount[$v['order_batch_goods_id']] = $v;
                }
            ...
            foreach ($forList as $id => $v) {
                ...
                if ($v['child']) {
                    foreach ($v['child'] as $x => $y) {
                       ...
                       if ($y['offers_type'] == 'buy-gift') {//赠品
                            unset($forList[$id]['child'][$x]['sale_price'], $forList[$id]['child'][$x]['amount']);
                            $forList[$id]['child'][$x]['goods_name'] = '<font color=red>[赠品]</font>' . $forList[$id]['child'][$x]['goods_name'];
                       } else if ($y['offers_type'] == 'fixed') {//商品特价
                            //$forList[$id]['child'][$x] = array('goods_name' => '<font color=red>[特价抵扣]</font>' . $y['goods_name'], 'amount' => $y['amount']);
                       } else if ($y['offers_type'] == 'discount') {//商品折扣
                            //$forList[$id]['child'][$x] = array('goods_name' => '<font color=red>[折扣抵扣]</font>' . $y['goods_name'], 'amount' => $y['amount']);
                       } else if ($y['offers_type'] == 'exclusive') {//专享
                            //$forList[$id]['child'][$x] = array('goods_name' => '<font color=red>[价格专享抵扣]</font>' . $y['goods_name'], 'amount' => $y['amount']);
                       }else if ($y['offers_type'] == 'price-exclusive') {//站内专享
                            //$forList[$id]['child'][$x] = array('goods_name' => '<font color=red>[价格专享抵扣]</font>' . $y['goods_name'], 'amount' => $y['amount']);
                       }
                    }
