<?php
class Shop_Models_API_Cart extends Custom_Model_Dbadv
{
    /**
     * 
    ...
    public function getCartProduct($cart = null)
    {
        ...
        if (is_array($cart) && count($cart)) {
            ...
            if (is_array($products['data']) && count($products['data'])) {
                foreach ($products['data'] as $k => $product) {
                    ...
                    //享受会员折扣 开始
                    $goodsInfo = $_goods -> getGoodsInfo(" and goods_id ={$product['goods_id']}");
                    $product['data'][$k]['offers_type'] = $goodsInfo['offers_type'];
                    if ($discount > 0 &&
                        $discount < 1 &&
                        $product['data'][$k]['offers_type'] != 'exclusive' &&
						$product['data'][$k]['offers_type'] != 'price-exclusive' &&
                        $product['data'][$k]['offers_type'] != 'discount') {
                        $products['data'][$k]['org_price'] = $products['data'][$k]['price'];
                        $products['data'][$k]['price_before_discount'] = $products['data'][$k]['price'];
                        $products['data'][$k]['remark'] = "生日";
                        $products['data'][$k]['price'] = $products['data'][$k]['price'] * $discount;
                        $products['data'][$k]['member_discount'] = $discount * 10;
                    }
                    //享受会员折扣 结束
        ...
        //组合商品
        $groupGoods = new Shop_Models_API_GroupGoods();
        $products = $groupGoods -> responseToCart($products);

        //团购
        $tuan = new Shop_Models_API_Tuan();
        $products = $tuan -> responseToCart($products);

        //活动
        $offers = new Shop_Models_API_Offers();
        $products = $offers -> responseToCart($products);
    ...
    public function showPayment($logistic= array())
    {
        ...
        if($this -> _uid =='8966'){//交通银行 
            $payment = $this -> getOnlyPayment('bankcomm');
            $defaultPayment='bankcomm';
        }
    ...
    public function addOrder($data, $unionInfo = array() ,$address = array())
    {
        ...
        if ($this -> user['ltype'] == 'phone') {
            $source = 2;
        } elseif ($this -> user['ltype'] == 'anonym') {
        	$source = 3;
        }
        else {
            $source = 1;
        ...
        if ($product) {                                                     //普通商品
            foreach($product as $data){
                ...
                if ($data['is_vitual']) {
                    if (!$this -> _cardAPI) {
                        $this -> _cardAPI = new Shop_Models_API_Card();
                    }
                    
                    for ($cardIndex = 1; $cardIndex <= $data['number']; $cardIndex++) {
                        $encode = Custom_Model_Encryption::getInstance() -> encrypt($tmp[$data['product_id']].$cardIndex, 'vitualCard');
                        $bodyCard = array('type' => 1,
                                          'product_id' => $data['product_id'],
                        ...
                        $this -> _cardAPI -> addBodyCard($bodyCard);
                    }
                }
            }
        }
        ...
        if ($offers) {
            foreach($offers as $offersID => $item) {
                foreach ($item as $i) {
				   ...
				   if($i['offers_id'] == '30'){
						setcookie('offers_limit', '1', time () + 86400 * 30, '/');
				   }
		
                    if ($i['offers_type'] == 'buy-gift' ||              //商品买赠
                        $i['offers_type'] == 'order-gift' ||            //订单买赠
                        $i['offers_type'] == 'order-buy-gift' ||        //购买某类商品赠送一个某产品
                        $i['offers_type'] == 'assign-goods' // 指定 商品
         			) {          
                        if ($i['product']) {
                            if ($i['is_group']) {
                                $groupGoodsAPI = new Shop_Models_API_GroupGoods();
                                $groupGoodsArray = $i['product'];
                                $i['product'] = '';
                                foreach ($groupGoodsArray as $p) {
                                    $tempGroupGoodsDatas = $groupGoodsAPI -> fetchConfigGoods(array('group_id' => $p['product_id']));
                                    foreach ($tempGroupGoodsDatas as $tempGroupGoodsData) {
                                        $i['product'][] = $tempGroupGoodsData;
                                    }
                                }
                            }
                            ...
                            foreach ($i['product'] as $p) {
                                $this -> _db -> addOrderBatchGoods(array('order_id' => $orderID,
                                                                        ...
                                                                        'batch_sn' => $orderSN,
                                                                        'type' => 1,
                                                                        'parent_id' => $i['offers_type'] == 'buy-gift' ? $tmp[$i['parent_pid']] : 0,
                                                                        'offers_id' =>  intval($offersID),
                                                                        'offers_name' => $i['offers_name'],
                                                                        'offers_type' => $i['offers_type'],
                                                                        'add_time' => $time,
                                                                        ...
                                                                        'height' => $p['p_height'],
                                                                        'weight' => $p['p_weight'],
                                                                        'number' => $p['number'] ? $p['number'] : 1,
                                                                        'price' => $i['org_price'],
                                                                        'sale_price' => $i['price']));
                                $stockAPI -> holdSaleProductStock($p['product_id'], $p['number'] ? $p['number'] : 1);
                            }
                        }
                    } else if(($i['offers_type'] == 'fixed' && !$i['is_group']) ||               //商品特价
                              $i['offers_type'] == 'discount' ||            //商品折扣
                              $i['offers_type'] == 'discount-adv' ||        //商品第N件折扣
                              $i['offers_type'] == 'exclusive' ||           //专享价格
							  $i['offers_type'] == 'price-exclusive' ||     //站内专享价格
                              $i['offers_type'] == 'buy-minus' ||           //商品立减
                              $i['offers_type'] == 'quantity-discounts' ||  			//多件商品购买折扣
                              $i['offers_type'] == 'tuan') {                //团购

                        if ($i['offers_type'] == 'tuan') {
                            $offersID = substr($offersID, 5, strlen($offersID));
                            $goodstype = 6;
                        }
                        else    $goodstype = 1;
                        $orderGoodsID = $this -> _db -> addOrderBatchGoods(array('order_id' => $orderID,
                                                                                 ...
                                                                                 'batch_sn' => $orderSN,
                                                                                 'type' => $goodstype,
                                                                                 'parent_id' => $tmp[$i['parent_pid']],
                                                                                 'offers_id' => $offersID,
                                                                                 'offers_name' => $i['offers_name'],
                                                                                 'offers_type' => $i['offers_type'],
                                                                                 'add_time' => $time,
                                                                                 'goods_name' => $i['offers_name'],
                                                                                 'goods_style' => $i['goods_style'],
                                                                                 'number' => $i['number'] ? $i['number'] : 1,
                                                                                 'price' => $i['price'],
                                                                                 'sale_price' => $i['price']));
                    } else if ($i['offers_type'] == 'minus') {              //订单立减
                        $orderGoodsID = $this -> _db -> addOrderBatchGoods(array('order_id' => $orderID,
                                                                                 'order_batch_id' => $orderBatchID,
                                                                                 'order_sn' => $orderSN,
                                                                                 'batch_sn' => $orderSN,
                                                                                 ...
                                                                                 'price' => $i['price'],
                                                                                 'sale_price' => $i['price']));
                    } else if ($i['offers_type'] == 'fixed-package' ||      //定额礼包
                               $i['offers_type'] == 'choose-package') {     //自选礼包
                        $orderGoodsID = $this -> _db -> addOrderBatchGoods(array('order_id' => $orderID,
                                                                                 'order_batch_id' => $orderBatchID,
                                                                                 'order_sn' => $orderSN,
                                                                                 'batch_sn' => $orderSN,
                        ...
                        if ($i['product']) {
                            foreach ($i['product'] as $p) {
                               $this -> _db -> addOrderBatchGoods(array('order_id' => $orderID,
                                                                        ...
                                                                        'price' => $p['org_price'],
                                                                        'sale_price' => $p['price']));
                                $stockAPI -> holdSaleProductStock($p['product_id'], 1);
                            }
                        }
                    }
                    unset($orderGoodsID);
        ...
        if ($cards){
            ...
            $cardObj -> useCard(array('user_id' => $this -> user['user_id'],
                                      'user_name' => $this -> user['user_name'],
                                      'batch_sn' => $orderSN,
                                      'add_time' => $time,
                                     )
        ...
        //api 插入联盟数据
        unset($data);
        if ($unionInfo['parent_id']) {
            $data['parent_id'] = $unionInfo['parent_id'];
            $data['parent_param'] = $unionInfo['parent_param'];
            $data['proportion'] = $unionInfo['proportion'] ? $unionInfo['proportion'] : 0;
            $data['calculate_type'] = $unionInfo['calculate_type'] ? $unionInfo['calculate_type'] : 1;//这行2012.2.9添加，（记录是否为“固定分成比率” or “分类分成比率”）
            ...
            $data['rank_name'] = $this -> user['rank_name'];
            $data['price_goods'] = $priceGoods;
            $data['price_order'] = $priceOrder;
            $data['affiliate_amount'] = $pricePay - $priceLogistic;
            ...
            //商品类别判断结束
            $_union = new Shop_Models_API_Union();
            $_union -> setAffiliate($data,$cart);
        }
   ...
   public function addOrderGiftCard($data,$unionInfo=Array())
   {
   	  ...
   	  if ($product) { 
   	  	foreach($product as $data){
   	  		...
   	  		$bodyCard = array('card_type' => 1,
   	  					...
   	  					//'user_id' => $this -> user['user_id'],
   	  					//'user_name' => $this -> user['user_name'],
   	  					'buyer_id' => $this -> user['user_id'],//购买人
   	  					'status' => 2,//购买未付款，付款后状态改为0 
	 ...
	 * */
     public function getLogisticPrice($product) {
		...
		//判断专享免运费
		if ( $priceLogistic > 0 && $product['offers'] ) {
			foreach ( $product['offers'] as $key => $off ) {
				foreach( $off as $k => $o ){
				    if ( $o['offers_type'] == 'tuan' )  continue;
					$order_offers[] = $key;
				}
			}
			
			if ( count($order_offers) >= 1 ) {
				$ids = implode(',',array_unique($order_offers));
				$offersAPI = new Shop_Models_API_Offers();
				$offers = $offersAPI->getOffers(" and offers_id in (" .$ids.")" );
				if($offers && count($offers) >=1 ){
					foreach($offers as $k=>$o){
						if ($o['config']['freight']=='1') {
						    if ($o['config']['freight_limit_number'] > 1) {
						        $offers_number = 0;
						        if ( $product['offers'][$o['offers_id']] ) {
    						        foreach ($product['offers'][$o['offers_id']] as $p) {
    						            $offers_number += $p['number'];
    						        }
    						    }
						        if ( $offers_number < $o['config']['freight_limit_number'] )    continue;
						    }else{
							  return $priceLogistic = 0;
							  break;
						    }
		...
		if($priceLogistic>0 && $product['offers']){
		    foreach($product['offers'] as $key=>$off){
		        foreach($off as $o) {
		            if ($o['offers_type'] != 'tuan')  continue;
		            if ( ($o['freight'] > 0) && ($altlogistic < $o['freight']) ) {
		                $altlogistic = $o['freight'];
		            }
		        }
		    }
		}
	 ...
	 {
		$infos = $this->_db->getWarehouseInfos();
		...
		foreach ($infos as $val) {
			$warehouse_infos[$val['warehouse_id']] = $val['warehouse_name']."&nbsp;&nbsp;&nbsp;&nbsp;自提地址:".$val['province_name'].$val['city_name'].$val['district_name'].$val['address'];
		}
