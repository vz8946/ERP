...
<div class="member" style="">
  ...
  <div class="memberright">
    <div class="ding">
	 ...
	 {{if $order.warehouse neq 0}}
	 <p>
	 自提点：{{$order.warehouse.warehouse_name}}&nbsp;&nbsp;&nbsp;&nbsp;{{$order.warehouse.province_name}}{{$order.warehouse.city_name}}{{$order.warehouse.district_name}}
	 </p>
	 {{/if}}
	 <p>发票信息
		{{if $order.invoice_type eq '0'}}不开发票{{elseif $order.invoice_type eq '1'}}个人：{{$order.invoice}}　证件号码:{{$order.licence}}　发票内容:{{$order.invoice_content}}{{elseif $order.invoice_type eq '2'}}单位:{{$order.invoice}}　发票内容:{{$order.invoice_content}}　税号:{{$order.Tariff}}{{/if}}
	 </p>
	</div>
	...
	{{if $order.status_logistic==0 && $order.status==0}}	
        <form method="post" name="addressForm" id="addressForm" action="{{url param.action=$action}}" onsubmit="return addressSubmit()" target="ifrmSubmit">
            <table width="720"  border="0"  cellpadding="0" cellspacing="0" class="publictable">
			    ...
			    <a style="color: #FF3300;">*</a>    -->                    </td>
                        <td width="86" height="35" align="right"><strong>收货人姓名：</strong></td>
                      <td width="270" height="35">{{$order.addr_consignee}}<!--<input type="text" name="addr_consignee" size="30" maxlength="40" value="{{$order.addr_consignee}}" class="istyle"/><a style="color: #FF3300;">*</a>--></td>
                    </tr>
                    <tr>
                        <td width="70" height="35" align="right"><strong>电&nbsp;&nbsp;&nbsp;&nbsp;话：</strong></td>
                        <td width="294" height="35">{{$order.addr_tel}}
                      ...
                      <td width="270" height="35">{{$order.addr_mobile}}<!--<input type="text" name="addr_mobile" size="30" maxlength="20" value="{{$order.addr_mobile}}"  class="istyle"/>--></td>
                    </tr>
                    <tr>
                        <td width="70" height="35" align="right"><strong>详细地址：</strong></td>
                      <td width="294" height="35">{{$order.addr_address}}<!--<input type="text" name="addr_address" size="30" maxlength="100" value="{{$order.addr_address}}"  class="istyle"/><a style="color: #FF3300;">*</a>--></td>
