           ...
           {{if $addressList}} 
            <ul>
            	...
            	<li {{if  $address_id eq $data.address_id}}class="active"{{/if}} id="add_item_{{$data.address_id}}" address="{{$data.address_id}}" onclick="">
                    ...
                    <div class="op_addr">
                    	<a href="javascript:;" onclick="editAddressInfo({{$data.address_id}},'edit','addr_gift');">修改</a>
        	            <a href="javascript:;" onclick="delAdress({{$data.address_id}},'addr_gift');">删除</a>
                    </div>
        ...
        <script type="text/javascript">
        	$(function(){
				$(".arrive_addr ul .wrap_addr").click(function(){
					...
					var aid= $(this).parent("li").attr('address')
					$("#address_id").val(aid);
					setAddress(aid,'addr_gfit');
				})
			})
        </script>
        
<div class="addr_add">      
<h3>{{if $address_id && $type eq 'edit'}}修改地址{{else}}<a href="javascript:;" onclick="editAddressInfo(0,'edit','addr_gift')">创建新地址</a>{{/if}}</h3>
<div id="address_form_box" style="{{if $type eq 'edit' || !$addressList}}display:block;{{else}}display:none;{{/if}}">
<form  action="/flow/edit-add-addr/" method="post" id="addressFrom">
  <table width="100%" border="0">
          ...
          <input type="hidden" name="tpl" value="addr_gift">
          <a class="btn_save" href="javascript:;"  onclick="return check_post_address('addr_gift');" >保存配送信息</a>
          </form>
  </div>     