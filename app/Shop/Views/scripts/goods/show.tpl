  ...
  <div class="product_info">
        ...
        <div class="summary">
             <p class="price">市场价:<span>￥{{$data.market_price}}</span></p>
		       {{if $data.org_price}}		 		
			 		 <p class="price">垦丰价:<span><em><b>￥</b>{{$data.org_price}}</em> </span></p>	
			 		 <p class="price_jiankang">
				    {{if $data.offers_type=='exclusive'}}
				       {{if $data.show_name}}{{$data.show_name}}专享价{{else}}专享价{{/if}}:<span>￥{{$data.price}}</span> {{if $data.excluisv_name neq ''}}<i>（{{$data.excluisv_name}}）</i>{{/if}}
					{{elseif $data.offers_type=='price-exclusive'}}
						试 用 价:<span>￥{{$data.price}}</span> {{if $data.excluisv_name neq ''}}<i>（{{$data.excluisv_name}}）</i>{{/if}}
				    {{elseif $data.offers_type=='fixed'}}	
					   <span>{{if $data.offers_message=='only_new_member'}}新会员特价 {{else}}抢购价 {{/if}}</span><em><b>￥</b>{{$data.price}}</em>				   			
					{{elseif $data.offers_type=='discount'}}
					    <span>{{$data.discount_title}} 折价</span><em><b>￥</b>{{$data.price}}</em>
					 {{/if}}
				        <i>已优惠￥{{math equation="x - y" x=$data.market_price y=$data.price}}</i>
				    </p>			    	
		  {{else}}
			  <p class="price_jiankang"><span>垦丰价</span><em><b>￥</b>{{$data.price}}</em><i>已优惠￥{{math equation="x - y" x=$data.market_price y=$data.price}}</i></p>	
           {{/if}}
					
					
				{{if $groupGoodsAData || $groupGoodsBData || $groupGoodsCData}}
					<div  class="other" style="padding:3px 0px;">
						...
						<div class="activity">
							...
							<dl class="yi">
								...
								<dd>
									<ol>
										{{foreach from=$groupGoodsBData item=groupGoods}}
										<li>
											<a href="/groupgoods-{{$groupGoods.group_id}}.html" target="_blank">{{$groupGoods.group_goods_name}}</a><em></em><strong class="red">￥{{$groupGoods.group_price}}</strong><a href="javascript:void(0)" onclick="addGroupCart({{$groupGoods.group_id}})" class="buttons">购买</a>
										</li>
										{{/foreach}}
									</ol>
								</dd>
							...
							<dl class="yi">
								...
								<dd>
									<ol>
										{{foreach from=$groupGoodsCData item=groupGoods}}
										<li>
											<a href="/groupgoods-{{$groupGoods.group_id}}.html" target="_blank">{{$groupGoods.group_goods_name}}</a><em></em><strong class="red">￥{{$groupGoods.group_price}}</strong><a href="javascript:void(0)" onclick="addGroupCart({{$groupGoods.group_id}})" class="buttons">购买</a>
										</li>
										{{/foreach}}
					...
					{{if $groupGoodsAData}}
				     <dl class="er">
								...
								<dd>
									<ol>
										{{foreach from=$groupGoodsAData item=groupGoods name="n"}}
										<li {{if $smarty.foreach.n.last}}style="border: none;"{{/if}} class="clear">
											<table width="100%">
												<tr>
													{{assign var="c" value=$groupGoods.group_goods_config|@count}}
													{{if $c == 0}}
													{{assign var=w value=50}}
													{{elseif $c == 1}}
													{{assign var=w value=105}}
													{{elseif $c == 2}}
													{{assign var=w value=165}}
													{{/if}}
													<td width="{{$w}}">
														<a href="/goods-{{$data.goods_id}}.html" target="_blank" title="{{$data.goods_name}}">
														<img width="40" src="{{$imgBaseUrl}}/{{$data.goods_img|replace:'.':'_60_60.'}}" />
														</a>
														{{if $groupGoods.group_goods_config}}
													  	<span>+</span>
														{{foreach from=$groupGoods.group_goods_config item=goods name=goods}}
														{{if $smarty.foreach.goods.iteration <= 2}}
														<a href="/goods-{{$goods.goods_id}}.html" target="_blank"  title="{{$goods.goods_name}}">
														<img width="40" src="{{$imgBaseUrl}}/{{$goods.goods_img|replace:'.':'_60_60.'}}" />
														</a>
														{{if $smarty.foreach.goods.iteration ne $groupGoods.group_goods_config|@count}}
													...
													<td>
														<a href="/groupgoods-{{$groupGoods.group_id}}.html" target="_blank">{{$groupGoods.group_goods_name}}</a> <strong class="red">￥{{$groupGoods.group_price}}</strong>
													</td>
													<td width="60">
														<a style="display: inline-block;" href="javascript:void(0)" onclick="addGroupCart({{$groupGoods.group_id}})" class="buttons">购买</a>
													</td>
												</tr>
											</table>
										</li>
		  ...
		  {{if $data.needLogin}}<font color="#999999">(该商品有活动优惠价，请先<a href="/auth/login/goto/{{$goto}}">登录</a>以获得最新价格)</font>{{/if}}</p>
          <p>品  牌：{{$brand.brand_name}}</p>
          <p>规  格：{{$data.goods_style}}</p>
          <p>
	  {{if $data.characters eq '1'}}适种积温带：第一积温带{{/if}}
	  {{if $data.characters eq '2'}}适种积温带：第二积温带{{/if}}
	  {{if $data.characters eq '3'}}适种积温带：第三积温带{{/if}}
	  {{if $data.characters eq '4'}}适种积温带：第四积温带{{/if}}
	  {{if $data.characters eq '5'}}适种积温带：第五积温带{{/if}}
	  {{if $data.characters eq '6'}}适种积温带：第六积温带{{/if}}
	  </p>
          <!--
          <p class="comment"><b>评  价：</b><i class="star02"><i class="star01"></i></i><span>已有<em>23</em>个人评价，100%好评</span></p>
           -->
				  ...
				  <div class="buyBox">
			            ...
			            <div class="link"><a href="javascript:void(0);" onclick="addToCart('{{$data.goods_sn}}','buy_now','{{$data.characters}}')" ><img width="101" height="35" src="{{$_static_}}/images/shop/btn_buy.jpg"></a>
			            <a href="javascript:void(0);" onclick="addToCart('{{$data.goods_sn}}','buy_cart','{{$data.characters}}')" ><img width="130" height="35" src="{{$_static_}}/images/shop/btn_cart.jpg"></a>
			...
			<div class="goods_left mains">
				...
				<ul class="title"  id="ttt">
					...
					<li  class="go_cart" style="margin-right: 0px;float: right;">
						<a class="add-cart"  href="javascript:void(0);" onclick="addCart('{{$data.goods_sn}}');">加入购物车</a>
					</li>
					-->
					<div style="clear:both;"></div>
				</ul>
					...
					<dl>
						<dd style="text-align:left">
						  ...
						  {{if $id eq '639'}}
							  ...
							  <a href="http://www.1jiankang.com/groupgoods-66.html" target="_blank"><img id="taozu01" src="{{$imgBaseUrl}}/images/special/mk0717/taozu01.jpg" width="245" height="279" alt="" /></a> 
							  <a href="http://www.1jiankang.com/groupgoods-62.html" target="_blank"><img id="taozu02" src="{{$imgBaseUrl}}/images/special/mk0717/taozu02.jpg" width="245" height="279" alt="" /></a> 
							  <a href="http://www.1jiankang.com/groupgoods-63.html" target="_blank"><img id="taozu03" src="{{$imgBaseUrl}}/images/special/mk0717/taozu03.jpg" width="245" height="279" alt="" /></a> 
