...
class GroupGoodsController extends Zend_Controller_Action
{
    /**
     * 
     * @var Shop_Models_API_GroupGoods
     */
    private $_api = null;
	/**
     * 初始化对象
	...
	{
		$this -> _api = new Shop_Models_API_GroupGoods();
	}
    ...
    public function showAction()
    {
		...
		if ($id > 0) {
                ...
                if($uid>0){
                    if($aid){
                         $viewTag = $this -> _api->getViewTag("type=2 and union_id='$uid' and union_param='$aid' and  goods_ids like '%$id%' " );
                    }else{
                         $viewTag = $this -> _api->getViewTag("type=2 and union_id='$uid' and goods_ids like '%$id%' " );
                    }
                    if($viewTag &&  in_array($id,  explode(',',$viewTag['goods_ids'])  )){
                        $this -> view -> showTag = '2';//组合商品
                        $this -> view -> viewTag = $viewTag;
                    }
                }
        }
        $offers = new Shop_Models_API_Offers();
        $this -> view -> data = array_shift( $offers -> responseToViewForGroup(array($this -> view -> data)) );
        //小导航
        ...
        if(count($group_goods)>1){
            ...
            $_goodsApi = new Shop_Models_API_Goods();
            $bfdshowgoods = $_goodsApi -> getTag(" tag_id = 22");
            $this -> view -> linkgoods = $bfdshowgoods['details'];
        }
        $this->view->css_more = ',list_v2.css';
        $this->view->js_more=',goods.js,check.js';
	    $this -> view -> page_title = "垦丰商城 - 世界种子精品商城";
		$this -> view -> page_keyword = "垦丰集团,垦丰电商,种子,玉米,水稻,大豆,小麦,大麦,甜菜,其他";
	    $this -> view -> page_description ="垦丰商城 -专业的种子产品商城，品质保障！";
    }
    /**
