...
<form name="searchForm" id="searchForm" action="/admin/group-goods/salelist">
...
<input type="button"  onclick="doExport(0)" name="dosearch" id="dosearch" value="查询"/></td>
	</tr>
</table>
</div>
</form>
...
<div class="content">
    ...
    <tr id="ajax_list{{$data.group_id}}">
        ...
        <td><input type="text" name="update" size="2" value="{{$data.group_sort}}" id="groupsort_{{$data.group_id}}" style="text-align:center;" onblur="groupSort({{$data.group_id}});"></td>
	...
	<td>{{$data.group_cost}}</td>
        ...
        <td id="status{{$data.group_id}}">{{if $data.status==0}}<a href="javascript:fGo()" onclick="changeStatus(0,{{$data.group_id}})" style=" color:red;">下架</a>{{else}}<a href="javascript:fGo()" onclick="changeStatus(1,{{$data.group_id}})" style=" color:blue;">上架</a>{{/if}}</td>
		<td id="status{{$data.group_id}}">{{if $data.check_status==1}}<a  style=" color:red;">已经审核</a>{{else}}<a style=" color:blue;">待审核</a>{{/if}}</td>
        <td> 
          <a href="javascript:fGo()" onclick="viewConfig({{$data.group_id}});">查看配置</a> | 
          <a href="javascript:fGo()" onclick="window.open('/shop/groupgoods-{{$data.group_id}}.html')">查看</a> | 
          <a href="javascript:fGo()" onclick="G('{{url param.action=sale-edit param.group_id=$data.group_id}}')">编辑</a> | 
        </td>
    </tr>
    {{/foreach}}
    </tbody>
