...
<form name="searchForm" id="searchForm" action="/admin/group-goods/check-list">
...
审核状态：<select name="check_status"><option value="" selected>请选择</option><option value="on" {{if $param.check_status eq 'on'}}selected{{/if}}>已经审核</option><option value="off" {{if $param.check_status eq 'off'}}selected{{/if}}>未审核</option></select>
...
[<a href="javascript:;" onclick="refreshStock();">更新库存</a>]</div>
<div class="content">
    ...
    <tr id="ajax_list{{$data.group_id}}">
       ...
       <td>{{$data.group_id}}</td>
        ...
        <td>{{$data.group_stock_number}}</td>
       		<td id="check_status{{$data.group_id}}">{{if $data.check_status==1}}已经审核{{else}}<a href="javascript:fGo()" onclick="changeCheckStatus(0,{{$data.group_id}})" >待审核</a>{{/if}}</td>
        <td> 
...
//更改状态
function changeCheckStatus(n,i){
	n=parseInt(n);
	if(n!=1 && n!=0){ n=0; }
	n=1-n;
	new Request({
		url:'/admin/group-goods/check-status/s/'+n+'/id/'+i,
		method:'get',
		onSuccess:function(data){
			data = JSON.decode(data);
			if(data.st == 'ok'){
				$('check_status'+i).innerHTML=data.html;
			}else{
				alert("更改失败，请稍后重试");
			}
		},
