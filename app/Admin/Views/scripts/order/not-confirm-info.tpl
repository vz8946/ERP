...
<tr>
  ...
  <td height="30">{{if $order.invoice_type eq '0'}}不开发票{{elseif $order.invoice_type eq '1'}}个人：{{$order.invoice}}　发票内容:{{$order.invoice_content}}{{elseif $order.invoice_type eq '2'}}单位:{{$order.invoice}}　发票内容:{{$order.invoice_content}}{{/if}}</td>
</tr>
</table>
...
{{if $order.lock_name==$auth['admin_name']}}
	<select name='pay_type' id="pay_type">
	    ...
	    {{else}}
		  {{if $order.user_id lt 10 && $order.user_id neq 5}}
		    ...
		    {{if $order.type eq 16}}
		      <option value="alipay" {{if $pay_type=='alipay'}}selected="selected"{{/if}}>支付宝</option>
		    {{/if}}
		  {{elseif $order.user_id eq 10 }}
		    <option value="cash" {{if $pay_type=='cash'}}selected="selected"{{/if}}>现金支付</option>
		    <option value="bank" {{if $pay_type=='bank'}}selected="selected"{{/if}}>银行打款</option>
		    <option value="external" {{if $pay_type=='external'}}selected="selected"{{/if}}>渠道支付</option>
		  {{else}}
...
<td height="30"><input type="text" name="price_logistic" id="price_logistic" value="{{$order.price_logistic|default:0}}" size="5"></td>
</tr>
...
<tr>
<th width="117" height="30"></th>
<td height="30"><input type="button" value="抵扣" onclick="confirmed('确定要抵扣吗?', $('myform'), '{{url param.action=add-gift-card-payment}}');" /></td>
</tr>
</table>
{{/if}}
{{/if}}
...
{{if $order.lock_name == $auth['admin_name']}}
  ...
  <td height="30">
    ...
    发票内容：
      ...
      <option value="种子" {{if $order.invoice_content eq '种子'}}selected{{/if}}>种子</option>
      <option value="农产品" {{if $order.invoice_content eq '农产品'}}selected{{/if}}>农产品</option>
...
function confirmed(str, obj, url) {
	var blance = parseFloat('{{$blance}}') + parseFloat($('price_logistic').value);
	if (isNaN(blance)) {
		blance = 0;
	}
