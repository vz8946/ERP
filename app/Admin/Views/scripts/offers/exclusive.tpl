...
<td>   
  ...
  <input type="checkbox" name="only_fix_num" value="1" {{if $offers.config.only_fix_num}}checked{{/if}}>只能购买指定的数量(超过该数量后，用户亦不能以正常价购买)
</td>
<td>是否免运费 *</td>
<td>
  ...
  当购买活动商品数量达到<input type="text" name="freight_limit_number" size="1" value="{{if $offers.config.freight_limit_number}}{{$offers.config.freight_limit_number}}{{else}}1{{/if}}">才生效</a>
</td>
</tr>
<tr>
</tr>
