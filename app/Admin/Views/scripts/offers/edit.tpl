...
<form name="myForm" id="myForm" action="{{url param.action=$action}}" method="post" onsubmit="return editSubmit()">
<div class="title">{{$title}}</div>
<div class="content">
<div class="sub_title">
    [ <a href="javascript:fGo()" onclick="G('{{url param.action=index}}')">返回活动列表</a> ]
</div>
<table cellpadding="0" cellspacing="0" border="0" width="100%" class="table_form">
<tbody>
<input type="hidden" name="offers_type" value="{{$offersType}}" />
<tr>
<td width="10%">活动名称 * </td>
<td width="40%"><input type="text" name="offers_name" size="25" maxlength="50" value="{{$offers.offers_name}}" msg="请填写活动名称" />    
     活动别名：<input type="text" name="as_name" size="25" maxlength="50" value="{{$offers.as_name}}" msg="请填写活动别名" /> </td>
<td width="10%">是否可使用礼券</td>
<td width="40%">
{{html_radios name="use_coupon" options=$canUseCoupon checked=$offers.use_coupon separator="<span style='padding-left:5px'></span>"}}
</td>
<!--
<td width="10%">适用会员等级</td>
<td width="40%">
{{*html_checkboxes name="offers_rank" options=$memberRanks checked=$offers.offers_rank separator="<span style='padding-left:5px'></span>"*}}
</td>
-->
</tr>
<tr>
...
<td width="40%">
  <span style="float:left;width:180px;">
    <input type="text" name="from_date" id="from_date" size="24" value="{{$offers.from_date}}"  class="Wdate" onClick="WdatePicker()" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
  </span>
</td>
<td width="10%">截至日期</td>
<td width="40%">
  <span style="float:left;width:180px;">
  <input type="text" name="to_date" id="to_date" size="24" value="{{$offers.to_date}}"  class="Wdate" onClick="WdatePicker()" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
  </span>
</td>
</tr>
</tbody>
</table>
{{include file="offers/`$offersType`.tpl"}}
</div>
