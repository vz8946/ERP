...
<div id="ajax_search_order">
        <form name="myform" id="myform" method="POST" action="/admin/shop/order-send">
        <input type="hidden" name="shop_id" value="{{$param.shop_id}}">
        ...
        <table cellpadding="0" cellspacing="0" border="0" align="center" id="sendButton" {{if !$allHaveLogisticNo}}style="display:none"{{/if}}>
          <tr>
            <td align="center">
              <input type="submit" name="beginSend" value="开始发货" onclick="if (confirm('该操作不可逆，是否真的要发货？')) return true;else return false;">
            </td>
          </tr>
        </table>
        </form>
...
function createLogisticNo()
{
    ...
    if (!isNaN(logistic_no)) {
        l = (logistic_no.length);
        logistic_no = parseInt(logistic_no, 10);
        logistic_no--;
    }
    ...
    for (i = 0; i < length; i++) {
        if (isNaN(logistic_no)) {
            ...
            str = logistic_no + '';
            if (str.length < l) {
                for (j = 0; j < l - str.length; j++) {
                    str = '0' + str;
                }
            }
            obj.rows[i].cells[5].innerHTML = str + '<input type="hidden" name="logistics[]" value="' + str + '">'; 
        }
    }
