...
<div class="search">
<form name="searchForm" id="searchForm" action="/admin/out-tuan/bill-clear">
...
<input type="hidden" name="fromwhere" value="caiwu" />
<table cellpadding="0" cellspacing="0" border="0" class="table">
    ...
    <tr id="row{{$data.id}}" >
        ...
        <td><input type="button" value="{{if $data.locker eq ''}}查看{{else}}操作{{/if}}" onclick="G('{{url param.action=bill-clear-do param.id=$data.id}}')" /></td>
		<td>{{$data.id}}</td>
    	<td>{{$data.bill_sn}}</td>
