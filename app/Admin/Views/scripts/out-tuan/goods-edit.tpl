...
<div class="content">
<form name="myForm" id="myForm" action="/admin/out-tuan/goods-edit" method="post" onSubmit="return checkThis()">
...
<table cellpadding="0" cellspacing="0" border="0" class="table" width="300">
  <tbody>
    <tr>
      ...
      <td>
        <select name="shopid" id="shopid">
          ...
          <option value="{{$shop.shop_id}}" {{if $shop.shop_id eq $detail.shop_id}}selected="selected"{{/if}}>{{$shop.shop_name}}</option>
          {{/foreach}}
        </select>
      </td>
    </tr>
    <tr>
      <td>商品名称</td>
      <td><input type="text" name="goods_name" id="goods_name" size="50" value="{{$detail.goods_name}}" /></td>
    </tr>
	<tr>
	  <td>商品类型（选择时时注意）</td>
	  <td><input type="radio" name="goods_type" value="0" {{if $detail.goods_type eq '0'}}checked="checked"{{/if}} />正常商品或组合商品<br /><input type="radio" name="goods_type" value="1" {{if $detail.goods_type eq '1'}}checked="checked"{{/if}} />任意数量商品（多个）比如拉手A商品n个，B商品n个</td>
	</tr>
	<tr>
	  <td>商品编码</td>
	...
	<tr>
	  <td>数量</td>
	  <td><input type="text" name="goods_number" id="goods_number" value="{{$detail.goods_number}}" /></td>
	</tr>
    <tr>
      <td>商品备注</td>
      <td><textarea name="goods_desc" id="goods_desc" cols="50" rows="6">{{$detail.goods_desc}}</textarea></td>
    </tr>
	<tr>
	  <td>商品售价</td>
	  <td><input type="text" name="goods_price" id="goods_price" value="{{$detail.goods_price}}" onkeyup="if(isNaN(value)){this.value='';}" onafterpaste="if(isNaN(value)){this.value='';}" /></td>
	</tr>
	<tr>
	  <td>供货价</td>
	  <td><input type="text" name="supply_price" id="supply_price" value="{{$detail.supply_price}}" onkeyup="if(isNaN(value)){this.value='';}" onafterpaste="if(isNaN(value)){this.value='';}" onblur="calaRate()" onblur="changePrice('supply_price', '{{$detail.price_limit}}')" /></td>
	</tr>
	<tr>
	  <td>费率</td>
	  <td><input type="text" name="rate" id="rate" value="{{$detail.rate}}" onkeyup="if(isNaN(value)){this.value='';}" onafterpaste="if(isNaN(value)){this.value='';}" />&nbsp;%</td>
	</tr>
    <tr>
      <td></td>
