...
{{/if}}
	...
	<div class="content">
		<div class="sub_title">[ <a href="javascript:fGo()" onclick="G('{{url param.action=addadvform}}')">添加广告位</a> ]</div>
		...
		<tr id="ajax_list{{$item.id}}">
			<td>{{$item.name}}</td>
			<td>
				<a href="{{$item.url}}" target="_blank"><img src="{{$imgBaseUrl}}/{{$item.imgUrl}}" height="50" width="100" /></a>
			</td>
			<td title="{{$item.position}}"><div style="width:350px;height:30px;overflow:hidden;margin:0;padding:0;">{{$item.position}}</div></td>
			<td >{{$item.grade}}</td>
			<td>{{if $item.isDisplay eq 1}}展示 <font style="color:#009966; cursor:pointer;" onclick="changHot({{$item.id}}, 0);">【隐藏】</font>{{elseif $item.isDisplay eq 0}}隐藏 <font style="color:#FF3300; cursor:pointer;" onclick="changHot({{$item.id}}, 1);">【展示】</font>{{/if}}</td>
			<td>
			<a href="javascript:fGo()" onclick="G('{{url param.action=editadvform param.id=$item.id}}')">编辑</a>||
			<a href="javascript:fGo()" onclick="reallydelete('{{url param.action=del-adv}}','{{$item.id}}','{{url param.action=listadvertising}}')">删除</a>
		</td>
		</tr>
		{{/foreach}}
		</tbody>
