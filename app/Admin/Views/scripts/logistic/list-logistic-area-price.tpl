<div class="search">
<form method="get" action="{{url param.action=list-logistic-area-price}}">
    <table>
        <tr>
            <td width=50>物流公司</td>
            <td width=200>
                <select name="logistic_code">
                    ...
                    <option value={{$data.logistic_code}} {{if $data.logistic_code==$logisticCode}}selected{{/if}}>{{$data.name}}</option>
                    {{/foreach}}
                </select>
            </td>
            <td width=50>地区</td>
...
<div class="content">
    ...
    <tr>
        ...
        <td width=150>城市</td>
        <td width=150>区县</td>
        {{foreach from=$title item=label}}
        <td>{{$label}}</td>
        {{/foreach}}
    </tr>
    </thead>
    <tbody>
    {{foreach from=$logisticArea item=data}}
    <tr>
        <td>
        <input type="button" value="编辑" onclick="G('{{url param.action=edit-logistic-area-price param.logistic_area_id=$data.logistic_area_id param.logistic_code=$logisticCode param.province_id=$provinceID param.city_id=$cityID param.area_id=$areaID}}')" />
        </td>
        <td>{{$data.country}}</td>
        <td>{{$data.province}}</td>
        <td>{{$data.city}}</td>
        <td>{{$data.area}}</td>
        {{foreach from=$data.price item=price}}
        <td>{{$price}}</td>
        {{/foreach}}
    </tr>
    {{/foreach}}
