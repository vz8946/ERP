...
class Admin_Models_DB_LogicArea
{
	...
	public function insert(array $data)
	{
		...
		$cnt = $this -> _db -> fetchOne("SELECT count(*) as cnt FROM `$this->_table_logic_status`");
		$r = $this -> _db -> fetchAll("SELECT product_id FROM `$this->_table_product` ORDER BY product_id DESC");
		for ($i = 1; $i <= $cnt; $i++) {
	        foreach ($r as $k => $v) {
	    		$stockStatus .= '('.$lastInsertId.', '.$v['product_id'].', '.$i.'),';
	    	}
