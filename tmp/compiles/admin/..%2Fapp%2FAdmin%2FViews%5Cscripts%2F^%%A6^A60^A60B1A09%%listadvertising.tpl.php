<?php /* Smarty version 2.6.19, created on 2014-10-24 19:28:52
         compiled from news/listadvertising.tpl */ ?>
<style type="text/css">
...
<div class="search" onclick="closeSearchBox();">
	<form name="searchForm" id="searchForm" action="/admin/news/listadvertising">
...
<?php endif; ?>
	...
	<div class="content">
		<div class="sub_title">[ <a href="javascript:fGo()" onclick="G('<?php echo $this -> callViewHelper('url', array(array('action'=>'addadvform',)));?>')">添加广告位</a> ]</div>
...
/<?php echo $this->_tpl_vars['item']['imgUrl']; ?>
...
"><div style="width:350px;height:30px;overflow:hidden;margin:0;padding:0;"><?php echo $this->_tpl_vars['item']['position']; ?>
			...
			<td><?php if ($this->_tpl_vars['item']['isDisplay'] == 1): ?>展示 <font style="color:#009966; cursor:pointer;" onclick="changHot(<?php echo $this->_tpl_vars['item']['id']; ?>
, 0);">【隐藏】</font><?php elseif ($this->_tpl_vars['item']['isDisplay'] == 0): ?>隐藏 <font style="color:#FF3300; cursor:pointer;" onclick="changHot(<?php echo $this->_tpl_vars['item']['id']; ?>
, 1);">【展示】</font><?php endif; ?></td>
			<td>
			<a href="javascript:fGo()" onclick="G('<?php echo $this -> callViewHelper('url', array(array('action'=>'editadvform','id'=>$this->_tpl_vars['item']['id'],)));?>')">编辑</a>||
			<a href="javascript:fGo()" onclick="reallydelete('<?php echo $this -> callViewHelper('url', array(array('action'=>"del-adv",)));?>','<?php echo $this->_tpl_vars['item']['id']; ?>
','<?php echo $this -> callViewHelper('url', array(array('action'=>'listadvertising',)));?>')">删除</a>
		</td>
		</tr>
...
function changHot(articleID, st){
	...
	if(st!=1 && st!=0){ st = 0; }
	new Request({
		url:'/admin/news/is-display/id/'+articleID+'/st/'+st,
