 ...
 * 
 * “程序”采用utf-8编码
 * 词库"dictjiankang.dat" 采用gbk编码
 * 单词条格式：关键词+空格+回车
 * 
 * 用法：
 * require("SplitWord.php");
 * $str = " 恒寿堂鲨鱼肝油赠金乳钙 ";
 * $str = iconv('utf-8', 'gbk', $str);//字符串转换成gbk
 * 
 * $sp = new SplitWord();
 * $rs = $sp->SplitRMM($str);
 * $rs = iconv('gbk', 'utf-8', $rs);//结果转成utf-8
 * 
 * $rs=恒寿堂 鲨鱼肝油 赠 金乳钙 
 */
class SplitWord
{
	...
	var $MaxLen = 25; //词典最大 12 （12*2+1）中文字，这里的数值为字节数组的最大索引
  ...
  function __construct(){  	  	
  	...
  	{
  		$this->OneNameDic[$this->CnOneName[$i].$this->CnOneName[$i+1].$this->CnOneName[$i+2]] = 1;
  		$i+=2;
  	}
  	...
  	//高级分词，预先载入词典以提分词高速度
  	$dicfile = dirname($_SERVER['DOCUMENT_ROOT'])."/data/dictjiankang.dat";
  	$fp = fopen($dicfile,'r');
  	while($line = fgets($fp,256)){
  		  $ws = explode(' ',$line);
  		  $this->TagDic[$ws[0]] = 1;
  		  $this->RankDic[strlen($ws[0])][$ws[0]] = 1;
  	}
  	fclose($fp);
  }
  ...
  function SetSource($str){
  	//原来的$this->SourceString = trim($this->ReviseString($str));
  	$this->SourceString = trim($str);
  	$this->ResultString = "";
  }
  ...
  function SplitRMM($str=""){
  	...
  	//如果“整个词”存在于词库中则直接返回
  	if($this->IsWord($this->SourceString)){return $this->SourceString;}
  	//对文本进行粗分
  	$this->SourceString = $this->ReviseString($this->SourceString);
  	//echo iconv('gbk', 'utf-8', $this->SourceString);exit;
  	//对特定文本进行分离
