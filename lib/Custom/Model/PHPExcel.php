...
set_include_path(dirname($_SERVER['DOCUMENT_ROOT']).DIRECTORY_SEPARATOR.'lib'.DIRECTORY_SEPARATOR.'Custom'.DIRECTORY_SEPARATOR.'Model'.PATH_SEPARATOR.get_include_path());
...
class Custom_Model_PHPExcel
{
    ...
    public function open($filename)
    {
        ...
        $this -> _reader = $reader;
        $this -> _handle = $reader -> load($filename);
        
        return true;
    }
    
    public function toArray($includeTitle = false, $sheet = 0)
    {
        if (!$this -> _reader || !$this -> _handle) {
            return false;
        }
        
        $currentSheet = $this -> _handle -> getSheet($sheet);
        
        $allColumn = $currentSheet->getHighestColumn();
        $allRow = $currentSheet->getHighestRow();
        
        for($currentRow = $includeTitle ? 1 : 2; $currentRow <= $allRow; $currentRow++) {
            $line = array();
            for($currentColumn = 'A'; $currentColumn <= $allColumn; $currentColumn++) {
                $line[] = $currentSheet -> getCellByColumnAndRow(ord($currentColumn) - 65, $currentRow) -> getFormattedValue();
            }
            $result[] = $line;
        }
