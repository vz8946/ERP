...
function PMA_SQP_parse($sql)
{
     ...
     */
    require 'Custom/Model/SqlParse/sqlparser.data.php';
        ...
        while ($count2 < $len) {
            ...
            */
            ...
            if (PMA_STR_isSqlIdentifier($c, false)
             || $c == '@'
             || ($c == '.'
              && ctype_digit(substr($sql, $count2 + 1, 1))
              && ($previous_was_space || $previous_was_bracket || $previous_was_listsep))) {
                ...
                */
                ...
                $is_digit                = !$is_identifier && !$is_sql_variable && ctype_digit($c);
                ...
                while (($count2 < $len) && PMA_STR_isSqlIdentifier(substr($sql, $count2, 1), ($is_sql_variable || $is_digit))) {
                    ...
                    if (($is_hex_digit && ctype_xdigit($c2))) {
                        $count2++;
                        continue;
                    } else {
                        $is_digit     = FALSE;
