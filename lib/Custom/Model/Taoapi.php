...
class Custom_Model_Taoapi
{
    ...
    public function __construct ()
    {
        if (self::$Taoapi_Util == NULL) {
            ...
            self::$Taoapi_Util = new Custom_Model_Taoapi_Util();
        }
         
		$Config = Custom_Model_Taoapi_Config::Init();

		$this->ApiConfig = $Config->getConfig();
		
        $this->Cache = new Custom_Model_Taoapi_Cache();
        ...
        if (! $this->taobaoData = $this->Cache->getCacheData($cacheid)) {
            ...
            if (isset($error['code'])) {
				...
				if($this->ApiConfig->RestNumberic && empty($this->_systemParam['apicount'])) {
                    ...
                    $this->_errorInfo = new Custom_Model_Taoapi_Exception($error, $tempParam, $this->ApiConfig->CloseError,$this->ApiConfig->Errorlog);
