...
function MagicZoom_getBounds(e){
	if(e.className=="bigImage")
		e=document.getElementById("bigImageBox");
...
function MagicZoom_withoutFirst(sequence,skip){result=[];for(var i=skip;i<sequence.length;i++)result.push(sequence[i]);return result};
...
function MagicZoom_stopEventPropagation(e){if(MagicZoom_ua=='gecko'||MagicZoom_ua=='safari'||MagicZoom_ua=='opera'){e.cancelBubble=true;e.preventDefault();e.stopPropagation()}else if(MagicZoom_ua=='msie'){window.event.cancelBubble=true}};
function MagicZoom(smallImageContId,smallImageId,bigImageContId,bigImageId,settings){
	...
	this.baseuri='';
	this.safariOnLoadStarted=false;MagicZoom_zooms.push(this);
...
MagicZoom.prototype.checkcoords=function(e){
	...
	if(MagicZoom_ua=='msie'){
		...
		while(tag&&tag.tagName!="BODY"&&tag.tagName!="HTML"){
			smallY+=tag.offsetTop;smallX+=tag.offsetLeft;
...
MagicZoom.prototype.initZoom=function(){
	...
	switch(this.settings['position']){
		...
		case'custom':this.bigImageCont.style.left='0px';
			this.bigImageCont.style.top='0px';
			break;
		case'inner':this.bigImageCont.style.left='0px';
			this.bigImageCont.style.top='0px';
			break;
		}
...
MagicZoom.prototype.replaceZoom=function(e,ael){
	...
	newBigImage.id=this.bigImage.id;newBigImage.src=ael.href;
	var p=this.bigImage.parentNode;p.replaceChild(newBigImage,this.bigImage);
...
function MagicZoom_findSelectors(id,zoom){var aels=window.document.getElementsByTagName("A");for(var i=0;i<aels.length;i++){if(aels[i].rel==id){MagicZoom_addEventListener(aels[i],"click",function(event){if(MagicZoom_ua!='msie'){this.blur()}else{window.focus()}MagicZoom_stopEventPropagation(event);return false});MagicZoom_addEventListener(aels[i],zoom.settings['thumb_change'],MagicZoom_createMethodReference(zoom,"replaceZoom",aels[i]));aels[i].style.outline='0';aels[i].mzextend=MagicZoom_extendElement;aels[i].mzextend({zoom:zoom,selectThisZoom:function(){this.zoom.replaceZoom(null,this)}});var img=document.createElement("IMG");img.src=aels[i].href;img.style.position='absolute';img.style.left='-10000px';img.style.top='-10000px';document.body.appendChild(img);img=document.createElement("IMG");img.src=aels[i].rev;img.style.position='absolute';img.style.left='-10000px';img.style.top='-10000px';document.body.appendChild(img)}}};
function MagicZoom_stopZooms(){while(MagicZoom_zooms.length>0){var zoom=MagicZoom_zooms.pop();zoom.stopZoom()}};
function MagicZoom_findZooms(){
	...
	for(var i=0;i<iels.length;i++){
		if(/MagicZoomLoading/.test(iels[i].className)){
			if(iels[i].alt!=''){
				loadingText=iels[i].alt;loadingImg=iels[i].src;
				break;
			}
		}
	}
	...
	for(var i=0;i<aels.length;i++){
		if(/MagicZoom/.test(aels[i].className)){
			while(aels[i].firstChild){if(aels[i].firstChild.tagName!='IMG'){
		...
		aels[i].style.position="relative";aels[i].style.display='block';
		...
		smallImg.id="sim"+rand;var bigCont=document.createElement("DIV");
		bigCont.id="bc"+rand;
		...
		matches=re.exec(aels[i].rel);var thumb_change='click';
		if(matches){
			thumb_change=matches[3];
		}
		re=new RegExp(/zoom\-width(\s+)?:(\s+)?(\w+)/i);matches=re.exec(aels[i].rel);
